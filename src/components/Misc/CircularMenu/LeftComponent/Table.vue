        ...
        <b-overlay :show="showOverlay" rounded="sm">
            <!-- <div class="row my-2">
                ...
                <div 
                    ...
                    <button class="btn-primary mt-1" type="submit" v-on:click="getUvaInflationIndexesFiltered()">Buscar</button>
                </div>
            </div>
            ...
            <!-- Table -->
            <div v-if="uvaInflationIndexes?.data?.length > 0">
                <b-table 
                    ref="uvaInflationIndexesTable"
                    class="margin-bottom"
                    striped
                    :busy.sync="isBusy"
                    :items="uvaInflationIndexes.data"
                    :fields="fields"
                    :bordered="true"
...
export default {
    ...
    async mounted() {
        //await this.getUvaInflationIndexes();
        
        // Mock
        this.uvaInflationIndexes = uvaInflationIndexesMock;
        
        this.uvaInflationIndexes.data.sort((a, b) => {
        ...
        this.uvaInflationIndexes.data = this.uvaInflationIndexes.data?.filter(item => item.value !== 0);

        this.totalRows = uvaInflationIndexesMock.data.length;
    ...
    methods: {
        ...
        async updateCurrentItemsCount(items) {
            ...
            this.currentItemsCount = this.uvaInflationIndexes.data.slice(start, end).length;
        },
        
        closeTag() {},

        async getUvaInflationIndexesFiltered() {
            // Filtrar por el texto ingresado en inputFilter y actualizar uvaInflationIndexes filtrado
            const filteredRates = this.uvaInflationIndexes.data.filter(rate => {
                return rate.entity.toLowerCase().includes(this.inputFilter.toLowerCase());
            });

            // Asignar los datos filtrados de nuevo a uvaInflationIndexes
            this.uvaInflationIndexes.data = filteredRates;
        },

        async getUvaInflationIndexes() {
            this.showOverlay = true;
            this.isBusy = true;

            await this.$axios.get(API_ROUTES.GET_INFLATION_INDEXES)
            .then(res => {
                this.uvaInflationIndexes = res.data;
